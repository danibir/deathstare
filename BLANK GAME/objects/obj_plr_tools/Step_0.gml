	...
	if host.liveinput != -4
		if host.state = "fight"
		{
			...
			if input.mb_leftpress and normalattackcooldown < 0
			{
				...
				var bullet = instance_create_layer(host.x, host.y - host.stats.stat_size * 16, "Instances", obj_bullet_bullethitbox)
				bullet.host = host
				
				if host.pointer != noone
				...
				{
					var item = struct_get(obj_eng_dictionary.itemstruct, items[i])
